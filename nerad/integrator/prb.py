...
@register_integrator("myprb")
class MyPRBIntegrator(mi.ad.common.RBIntegrator):
    r"""
    PRB code tranlated from Mitsuba C++.
    ...
    def __init__(self, props: mi.Properties):
        ...
        # This line is added because at the moment the built-in mitsuba function
        # eval_emitter_direction() is not been implemented yet.
        # For NeRF scenes when not optimizing envmap,
        # we need to disable this call manually in the configs passed to the training!
        self.allow_eval_emitter_call = props.get("config").dict.get("allow_eval_direction_call", True)
    ...
    def sample(self,
        ...
        loop = mi.Loop(name=f"Path Replay Backpropagation ({mode.name})",
                       state=lambda: (sampler, ray, depth, L, δL, β, η, active,
                                      prev_si, prev_bsdf_pdf, valid_ray, prev_bsdf_delta))
        ...
        while loop(active):
            ...
            with dr.resume_grad(when=not primal):
                if self.allow_eval_emitter_call and not primal:
            ...
            rr_prob = dr.minimum(β_max * η**2, self.rr_prob)
        ...
        return (
            ...
            dr.select(valid_ray, L if primal else δL, mi.Spectrum(0)),
            dr.neq(depth, 0),    # Ray validity flag for alpha blending
            # State for the differential phase
            dr.select(valid_ray, L, mi.Spectrum(0))
        )
