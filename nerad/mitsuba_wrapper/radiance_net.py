...
class RadianceMLP(nn.Module):
    def __init__(
        ...
        reflectance_input: bool
    ):
        super().__init__()
        self.reflectance_input = reflectance_input
        self.embedding = TcnnEmbedding(embedding)
        in_size = 3 + 3 + 3 + self.embedding.n_output_dims
    ...
    def forward(self, points, dirs, normals, albedo):
        net_in = torch.cat(
            [points, normals, dirs, self.embedding(points)],
            dim=-1,
        )
        if self.reflectance_input:
            net_in = torch.cat(
                [net_in, albedo],
                dim=-1,
            )
...
class MitsubaRadianceNetworkWrapper(MitsubaWrapper):
    def __init__(
        ...
        self.network = RadianceMLP(width, hidden, embedding, reflectance_input)
